...
- [x] Isolamento multi-tenant por Fornecedor
...
| Termo | Definição |
|-------|-----------|
| **Fornecedor** | Unidade lógica de isolamento multi-tenant (tenant) |
...
### RN-RF017-01 — Código Único por Fornecedor

**Descrição**: O código do centro de custo, departamento, setor ou seção deve ser único dentro do Fornecedor.
...
- Mensagem de erro: "Código {codigo} já existe para este Fornecedor"
...
### RN-RF017-09 — Isolamento por Fornecedor (Multi-tenancy)

**Descrição**: Todas as consultas e operações devem filtrar automaticamente por Id_Fornecedor do usuário autenticado.

**Justificativa**: Segurança e isolamento de dados multi-tenant.

**Critério de Aceite**:
- Tentativa de acesso a dados de outro Fornecedor → HTTP 403 (Forbidden)
- Filtro automático aplicado em todas as queries
- Auditoria deve registrar Id_Fornecedor
...
|--------|---------------|---------|
| **centro_custo.criado** | Após criação de Centro de Custo | { id, codigo, nome, filial_id, Fornecedor_id } |
...
- Nenhuma operação pode violar isolamento de Fornecedor (multi-tenant)
...
- Todas as consultas devem aplicar filtro de Fornecedor automaticamente
...
- Token deve conter: Id_Usuario, Id_Fornecedor, Roles, Permissions
