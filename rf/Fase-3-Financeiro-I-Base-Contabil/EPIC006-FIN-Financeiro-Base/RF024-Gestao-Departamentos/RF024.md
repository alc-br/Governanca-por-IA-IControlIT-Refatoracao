...
**Descrição**: Código departamento DEVE ser único por conglomerado, formato alfanumérico [TIPO]-[NOME] 3-30 caracteres.
...
- Backend valida unicidade por conglomerado

**Validação**:
- FluentValidation: `RuleFor(x => x.CodigoDepartamento).Matches(@"^[A-Z]{3,5}-[A-Z0-9]{2,20}$")`
- Database constraint: UNIQUE (Codigo_Departamento, Id_Conglomerado)
