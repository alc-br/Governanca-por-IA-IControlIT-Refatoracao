...
### RN-RF019-001: Código Único por Conglomerado

**Descrição**: O código do tipo de ativo deve ser único dentro do conglomerado.
...
- Todas as queries filtram por Id_Conglomerado do usuário autenticado
...
```http
GET /api/ativos/tipos?idConglomerado={guid}&categoria={string}&apenasAtivos={bool}&pageNumber={int}&pageSize={int}
...
```http
GET /api/ativos/tipos/hierarquia?idConglomerado={guid}
Authorization: Bearer {token}
```
...
```http
POST /api/ativos/tipos/importar?idConglomerado={guid}
Authorization: Bearer {token}
...
- `Id_Conglomerado` (UNIQUEIDENTIFIER) - Multi-tenancy obrigatório
...
- **RF-SYS-002 (Multi-Tenancy)** - Obrigatório para isolamento por Id_Conglomerado
...
|-----------|-------------|
| Entity | `backend/IControlIT.API/src/Domain/Entities/AtivoTipo.cs` |
| Commands | `backend/IControlIT.API/src/Application/Commands/AtivoTipo/*.cs` |
| Queries | `backend/IControlIT.API/src/Application/Queries/AtivoTipo/*.cs` |
| Controller | `backend/IControlIT.API/src/Web/Controllers/AtivoTipoController.cs` |
| Component Angular | `frontend/icontrolit-app/src/app/modules/cadastros/ativo-tipos/*.ts` |
