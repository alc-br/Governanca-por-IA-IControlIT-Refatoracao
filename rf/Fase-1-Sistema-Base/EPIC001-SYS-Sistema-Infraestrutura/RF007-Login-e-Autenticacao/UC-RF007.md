...
1. **Usuário acessa /sign-in**
   - GET https://app.icontrolit.com.br/sign-in
   - Frontend renderiza tela de login

2. **Usuário preenche email**
   - Exemplo: `admin@vale.com.br`
   - Frontend monitora campo email (onBlur ou onChange)

3. **Sistema identifica ClienteId pelo domínio do email OU campo hidden (multi-domínio)**
   - Se domínio único: extrai `@vale.com.br` → busca Cliente por domínio
   - Se multi-domínio: usuário seleciona Cliente em dropdown (campo hidden)
   - Frontend armazena ClienteId temporariamente

4. **Sistema busca Cliente.LogoUrl via ClienteId**
   - GET /api/clientes/{clienteId}/logo (opcional, antes do login)
   - OU aguarda até autenticação bem-sucedida (POST /login retorna logoUrl)

5. **Se logoUrl existe e válida: exibir logo corporativa do Cliente**
   - Frontend substitui logo padrão IControlIT pela logo do Cliente
   - Valida que URL é acessível (não 404, não timeout)

6. **Se logoUrl não existe ou inválida: exibir logo padrão IControlIT**
   - Fallback automático (não bloqueia login)
   - Nenhum erro visível ao usuário

7. **Usuário preenche senha e submete formulário**
   - Campos: email, senha
...
8. **Sistema valida credenciais**
   - POST /api/auth/login
   - Request: `{ email, senha }`
   - Validação backend: RN-AUTH-001

9. **Sistema retorna JWT + ClienteId + LogoUrl**
   - Response:
     ```json
     {
       "token": "eyJhbGc...",
       ...
       "userId": "guid",
       "clienteId": "guid",
       "logoUrl": "https://storage.icontrolit.com.br/clientes/vale/logo.png"
     }
     ```
   - logoUrl do Cliente (tenant), NÃO do usuário individual

10. **Frontend armazena logoUrl no localStorage**
    - Chave: `cliente_logo_url`
    - Persistência durante toda a sessão
    - Sobrescreve logo anterior (se troca de tenant)

11. **Sistema redireciona para /dashboard**
    - Navegação automática após login bem-sucedido

12. **Header do dashboard exibe logo corporativa do Cliente**
    - Logo persistente em toda a sessão
    - TODOS os usuários do mesmo Cliente veem a MESMA logo
    - Fallback para logo padrão se localStorage vazio

---

### Fluxos Alternativos

**FA-UC09-01: Não consegue identificar ClienteId antes do login**
- **Condição:** Domínio não registrado ou multi-domínio sem seleção
- **Ação:** Exibir logo padrão IControlIT
- **Resultado:** Após autenticação, sistema identifica ClienteId e retorna logoUrl
- **Comportamento:** Logo atualiza APÓS login, não bloqueia autenticação

**FA-UC09-02: LogoUrl aponta para arquivo não acessível (404, timeout)**
- **Condição:** Cliente.LogoUrl existe mas arquivo não carrega
- **Ação:** Fallback automático para logo padrão IControlIT
- **Resultado:** Login NÃO é bloqueado
- **Log:** Registrar warning no console: "Logo corporativa indisponível, usando logo padrão"

---

### Fluxos de Exceção

**FE-UC09-01: Credenciais inválidas**
- **Condição:** Email não encontrado OU senha incorreta
- **Resposta:** HTTP 401
- **Mensagem:** "Email ou senha incorretos" (RN-AUTH-011)
- **Observação:** Logo NÃO interfere em autenticação

**FE-UC09-02: Cliente desativado (FlAtivo=false)**
- **Condição:** Cliente.FlAtivo = false
- **Resposta:** HTTP 403
- **Mensagem:** "Cliente desativado. Contate o administrador."

**FE-UC09-03: Usuário desativado (FlAtivo=false)**
- **Condição:** Usuario.FlAtivo = false
- **Resposta:** HTTP 403
...
- **Response:**
  ```json
  {
    ...
    "clienteId": "660f9511-f3ac-52e5-b827-557766551111",
    "logoUrl": "https://storage.icontrolit.com.br/clientes/vale/logo.png"
  }
  ```
- **Observação:** logoUrl do Cliente (tenant), NÃO do usuário

**GET /api/clientes/{clienteId}/logo** (Opcional)
- **Request:** GET /api/clientes/660f9511-f3ac-52e5-b827-557766551111/logo
- **Response:**
  ```json
  {
    "logoUrl": "https://storage.icontrolit.com.br/clientes/vale/logo.png"
  ...
  ```
- **Quando usar:** Se precisar exibir logo ANTES do login (após identificar ClienteId pelo domínio)
...
- **Persistência:** logoUrl armazenada em localStorage (`cliente_logo_url`)
- **Visibilidade:** Logo exibida durante toda a sessão (header persistente)
- **Fallback:** Se logo inacessível, exibe logo padrão IControlIT (não bloqueia login)

---

### Observações Importantes

1. **Logo é do CLIENTE (tenant), NÃO do usuário individual**
   - TODOS os usuários de um mesmo Cliente veem a MESMA logo corporativa
   - Exemplo: 200 usuários da Vale → TODOS veem logo da Vale

2. **Logo NÃO interfere em autenticação**
   - Falha ao carregar logo → fallback para logo padrão
   - Login prossegue normalmente

3. **Identificação de ClienteId**
   - Preferencialmente por domínio do email (mais automático)
   - Alternativamente por seleção em dropdown (multi-domínio)

4. **Persistência da logo**
   - localStorage garante que logo persista durante navegação
   - Sobrescreve ao trocar de tenant (mesmo navegador)
...
### Job 1: Email de Recuperação de Senha

**UC:** UC01 - Solicitar Recuperação de Senha
**RN:** RN-AUTH-006 (Token de Recuperação Válido por 24 Horas)
**Gatilho:** POST /api/auth/forgot-password
**Processamento:** Hangfire job enfileirado
**SLA:** 5 minutos
...
```json
{
  ...
  "resetLink": "https://app.icontrolit.com.br/reset-password?token=xxx"
}
```

**Ações:**
1. Valida que email existe
2. Gera token de recuperação (24h)
3. Persiste em `TokenRecuperacaoSenha`
4. Envia email com link
5. Registra auditoria `AUTH_PASSWORD_RECOVERY_SENT`

**Retry Policy:** 3 tentativas com backoff exponencial (1min, 5min, 15min)

---

### Job 2: Notificação de Login em Novo Dispositivo

**UC:** UC00 - Realizar Login (FA-07)
**RN:** RN-AUTH-012 (Notificação de Login em Novo Dispositivo)
**Gatilho:** Login bem-sucedido com IP/UserAgent desconhecido
**Processamento:** Hangfire job enfileirado
**SLA:** 5 minutos
...
**Ações:**
1. Detecta IP/UserAgent novo (não registrado nos últimos 90 dias)
2. Busca geolocalização por IP (API externa)
3. Envia email de alerta ao usuário
4. Registra dispositivo em `DispositivosConhecidos`
5. Registra auditoria `AUTH_NEW_DEVICE_ALERT_SENT`

**Retry Policy:** 3 tentativas com backoff exponencial

**Observação:** Job **NÃO bloqueia** o login. É apenas notificação informativa.
...
|--------|------|-------|-----------|
| 2.1 | 2026-01-01 | Claude Sonnet 4.5 | Adicionada seção "Jobs Background" (2 jobs documentados) |
| 2.0 | 2025-12-31 | Agência ALC | Adequação completa UC v2.0 |
