...
- [x] Definir e monitorar limites de uso por Fornecedor
...
- [x] Isolamento multi-tenant (parâmetros por Fornecedor ou globais)
...
| **Limite de Uso** | Restrição quantitativa por Fornecedor (usuários, ativos, storage, API calls) |
| **Tenant (Fornecedor)** | Unidade lógica de isolamento de dados multi-tenant |
...
4. **Limites de Uso**: Definir e monitorar limites por Fornecedor (usuários, ativos, storage, API calls/dia)
...
**Descrição**: O código do parâmetro (`Cd_Parametro`) deve ser único por Fornecedor.

**Justificativa**: Prevenir duplicação de parâmetros e garantir referência inequívoca.

**Critério de Aceite**:
- Tentativa de criar parâmetro com código duplicado no mesmo Fornecedor → rejeição HTTP 409 (Conflict)
- Constraint UNIQUE no banco de dados
- Mensagem clara: `"Já existe um parâmetro com o código '{codigo}' neste Fornecedor"`
...
### RN-SYS-001-08 — Feature Flags por Fornecedor

**Descrição**: Feature flags podem ser globais (`Id_Fornecedor = NULL`) ou específicas por Fornecedor.

**Critério de Aceite**:
- Ao verificar se feature está ativa, verificar primeiro se existe flag específica do Fornecedor
...
**Descrição**: Apenas uma configuração de e-mail pode ser marcada como principal (`Fl_Principal = 1`) por Fornecedor.

**Critério de Aceite**:
- Ao marcar configuração como principal, desmarcar automaticamente outras do mesmo Fornecedor
...
### RN-SYS-001-12 — Limites de Uso por Fornecedor

**Descrição**: Cada tipo de limite (`Tipo_Limite`) deve ser único por Fornecedor.

**Critério de Aceite**:
- Constraint UNIQUE em (`Id_Fornecedor`, `Tipo_Limite`)
...
**Descrição**: Todos os parâmetros e configurações devem ser isolados por Fornecedor. Usuário só pode acessar dados do seu próprio Fornecedor.

**Critério de Aceite**:
- Filtrar sempre por `Id_Fornecedor` do usuário autenticado em queries
- Validar que `Id_Fornecedor` no payload coincide com Fornecedor do usuário autenticado
- Tentativa de acesso cross-tenant → rejeição HTTP 403 (Forbidden)
- Super Administrador pode acessar todos os Fornecedores (bypassar filtro)
...
- **Isolamento Multi-tenant**: Row-Level Security baseado em `Id_Fornecedor`
- **Criptografia**: AES-256 para dados sensíveis em repouso; TLS 1.2+ para dados em trânsito
- **Rate Limiting**: Limitar chamadas API por usuário/Fornecedor
