...
- [x] Isolamento multi-tenant (login e email únicos por conglomerado)
...
### RN-RF012-01 — Login Único por Conglomerado

**Descrição**: O login do usuário deve ser único dentro do conglomerado (multi-tenancy).

**Justificativa**: Garantir identificação inequívoca do usuário dentro do tenant, permitindo mesmo login em conglomerados diferentes.

**Critério de Aceite**:
- Constraint no banco: `UNIQUE (Login, Id_Conglomerado)`
- Validação case-insensitive
- Formato permitido: letras, números, '.', '_' ou '-'
- Tentativa de criar login duplicado no mesmo conglomerado → rejeição HTTP 409
...
**Regra**:
- **Access Token (JWT)**: Válido por 8 horas, contém claims (ID usuário, perfil, conglomerado, permissões)
...
### RN-RF012-15 — E-mail Único por Conglomerado

**Descrição**: E-mail do usuário deve ser único dentro do conglomerado.

**Regra**:
- Constraint no banco: `UNIQUE (Email, Id_Conglomerado)`
- Validação case-insensitive
...
**Critério de Aceite**:
- Tentativa de criar usuário com e-mail duplicado no mesmo conglomerado → HTTP 409
- E-mail inválido → HTTP 400 com mensagem específica
- E-mails em conglomerados diferentes → permitido
...
|--------|---------------|---------|
| **usuario.criado** | Após criação de usuário | `{ id, login, email, conglomerado_id, perfil_id }` |
...
- **Isolamento Multi-Tenant**: Nenhuma operação pode violar isolamento de conglomerado
...
- Filtro automático por `Id_Conglomerado` em todas as queries
- Row-Level Security (RLS) no banco de dados
- Impossibilidade de acesso cruzado entre conglomerados
