...
1. **Read Foundation Documents**:
   ...
   - Read `D:\DocumentosIC2\ERROS-A-EVITAR.md` to check if this is a known issue with documented solution
   - Read `D:\DocumentosIC2\ERROS-COMUNS-ANGULAR.md` if the issue involves frontend
...
- ❓ Múltiplas causas raiz possíveis e não sei qual priorizar
- ❓ Bug exige mudanças arquiteturais significativas
- ❓ Dúvida entre workaround temporário vs correção definitiva
- ❓ Correção impacta múltiplos RFs ou módulos
- ❓ Preciso decidir prioridade entre vários bugs críticos
- ❓ Encontrei conflito entre padrão atual e solução necessária
