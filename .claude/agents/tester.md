...
3. **D:\DocumentosIC2\ERROS-A-EVITAR.md** - Knowledge Base
...
2. Save token to .temp_claude/token.txt
...
- ❓ Testes falham mas não está claro se devo corrigir código ou ajustar testes
- ❓ Múltiplos bugs encontrados e preciso priorizar qual corrigir primeiro
- ❓ Preciso decidir nível de cobertura aceitável (quanto é suficiente?)
- ❓ Bateria de testes travada e não sei se continuo ou reporto ao usuário
- ❓ Conflito entre teste documentado e comportamento real do sistema
- ❓ Dúvida se bug é crítico o suficiente para bloquear aprovação
