...
**Requisito Funcional:** RF012 - Gestão de Usuários do Sistema
...
- Usuário possui permissão `CAD.USUARIOS.VIEW_ANY`
...
### FA-UC00-004: Nenhum Usuário Encontrado
...
### FE-UC00-002: Permissão Insuficiente
**Quando:** Usuário não possui permissão `CAD.USUARIOS.VIEW_ANY` (pré-condição)
...
1. Verificar permissão `CAD.USUARIOS.VIEW_ANY` via `AuthorizationPolicies.UsersRead`
...
**Pré-condição:** Usuário sem permissão `CAD.USUARIOS.VIEW_ANY`
**Passos:**
...
2. Usuário deve ter permissão `CAD.USUARIOS.CREATE`
...
### FA-UC01-001: Criar Usuário Inativo
...
### FA-UC01-002: Criar Usuário com Múltiplos Perfis
...
### FA-UC01-003: Criar Usuário sem Telefone
...
### FA-UC01-005: Gerar Senha Automática
...
### FE-UC01-003: Confirmação de Senha Diferente
...
### FE-UC01-005: Perfil Sem Permissão
...
### FE-UC01-006: Erro de Conexão com Banco
...
### FE-UC01-008: Campos Obrigatórios Vazios
...
|-----------|----------------|----------|
| `CAD.USUARIOS.CREATE` | Criar usuário | Super Admin, Admin |
| `CAD.USUARIOS.VIEW_ANY` | Ver lista/detalhes | Todos exceto Sem Permissões |
| `CAD.PERFIS.VIEW_ANY` | Listar perfis no formulário | Todos exceto Sem Permissões |
| `companies:company:read` | Listar empresas no formulário | Todos |

### Combinações de Permissões para Teste

| Cenário | CAD.USUARIOS.CREATE | CAD.USUARIOS.VIEW_ANY | CAD.PERFIS.VIEW_ANY | Resultado Esperado |
|---------|------------------|----------------|-----------------|-------------------|
...
| **Atores** | Super Admin, Admin, Gerente, Usuário (com permissão `CAD.USUARIOS.VIEW_ANY`) |
| **Pré-condições** | Usuário autenticado com permissão `CAD.USUARIOS.VIEW_ANY` |
...
- **Visualizar lista**: `CAD.USUARIOS.VIEW_ANY`
- **Visualizar detalhes**: `CAD.USUARIOS.VIEW_ANY`
- **Exportar lista**: `CAD.USUARIOS.VIEW_ANY` + `AUDIT.LOGS.EXPORT` (opcional)
...
2. Sistema valida permissão `CAD.USUARIOS.VIEW_ANY`
...
3. Sistema valida permissão `CAD.USUARIOS.VIEW_ANY`
...
7. Sistema exibe botões de ação (se autorizado):
   - "Editar" (requer `CAD.USUARIOS.UPDATE`)
   - "Excluir" (requer `CAD.USUARIOS.DELETE`)
   - "Resetar Senha" (requer `CAD.USUARIOS.UPDATE`)
   - "Ativar/Desativar" (requer `CAD.USUARIOS.UPDATE`)
...
- **Condição**: Usuário sem `CAD.USUARIOS.VIEW_ANY`
...
**Pré-condições**:
- Usuário logado com `CAD.USUARIOS.VIEW_ANY`
...
```json
{
  ...
  "permissions": ["CAD.USUARIOS.CREATE", "CAD.USUARIOS.VIEW_ANY", ...],
  "ativo": true,
...
**Pré-condições**:
- Usuário comum logado (sem `CAD.USUARIOS.VIEW_ANY` para outros)
...
**Pré-condições**:
- Usuário logado SEM `CAD.USUARIOS.VIEW_ANY`
...
| **Atores** | Super Admin, Admin, Gerente (com permissão `CAD.USUARIOS.UPDATE`) |
...
### Permissões Necessárias
- **Editar usuário**: `CAD.USUARIOS.UPDATE`
- **Trocar perfis**: `CAD.USUARIOS.UPDATE` + `CAD.PERFIS.VIEW_ANY`
- **Ativar/desativar**: `CAD.USUARIOS.UPDATE`
- **Resetar senha**: `CAD.USUARIOS.UPDATE`
...
2. Sistema valida permissão `CAD.USUARIOS.UPDATE`
...
**Pré-condições**:
- Admin logado com `CAD.USUARIOS.UPDATE`
...
**Pré-condições**: Usuário SEM `CAD.USUARIOS.UPDATE`
...
**Cenário**: Usuário com `CAD.USUARIOS.VIEW_ANY` mas SEM `update`
...
| **Atores** | Super Admin, Admin (com permissão `CAD.USUARIOS.DELETE`) |
| **Pré-condições** | Usuário autenticado, target user existe, permissão adequada |
...
### Permissões Necessárias
- **Deletar usuário**: `CAD.USUARIOS.DELETE`
- **Anonimizar (LGPD)**: `CAD.USUARIOS.DELETE` + `CAD.USUARIOS.ANONYMIZE` (opcional)
...
8. Sistema valida permissão `CAD.USUARIOS.DELETE`
...
**FE-01: Permissão Negada**
- **Condição**: Usuário sem `CAD.USUARIOS.DELETE`
- **Ação**:
  - Status: 403 Forbidden
...
**Pré-condições**:
- Admin logado com `CAD.USUARIOS.DELETE`
...
**Pré-condições**: Usuário SEM `CAD.USUARIOS.DELETE`
...
**Cenário**: Admin tem `CAD.USUARIOS.DELETE` mas NÃO tem `CAD.USUARIOS.ANONYMIZE`
...
**Validação**: Usuário deve ter `CAD.USUARIOS.UPDATE` (ou `delete`)
