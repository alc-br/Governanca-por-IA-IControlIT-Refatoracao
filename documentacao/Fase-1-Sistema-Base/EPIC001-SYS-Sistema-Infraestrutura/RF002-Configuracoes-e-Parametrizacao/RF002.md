...
- [X] Hierarquia multi-tenant (Global → Fornecedor → Empresa → Departamento → Usuário)
...
| **Configuração Multi-Tenant** | Específica por Fornecedor (ex: SMTP por cliente) |
...
**Descrição**: Configurações seguem hierarquia de precedência: Usuário → Departamento → Empresa → Fornecedor → Global.
...
- Nenhuma operação pode violar isolamento multi-tenant (configuração de um Fornecedor não pode ser acessada por outro)
...
- Row-Level Security: `WHERE Id_Fornecedor = @FornecedorAtual`
- Configurações globais: `Id_Fornecedor IS NULL`
