...
**RF:** RF030 — Gestão de Parâmetros de Faturamento
**Versão:** 2.0
**Data:** 2025-12-31
...
| UC09 | Criar Template de Rateio | Criar template reutilizável usando fórmulas NCalc |
| UC10 | Simular Rateio | Executar simulação com dados mock e exibir preview |
| UC11 | Criar Parâmetro Fiscal | Criar parâmetro com validação vigência e alíquota |
| UC12 | Atualizar Parâmetro Fiscal | Editar com versionamento e validação de impacto |
| UC13 | Homologar Fornecedor | Validar checklist documentos antes de liberar |
| UC14 | Dashboard de Qualidade | Exibir KPIs em tempo real com SignalR e cache Redis |
...
### Descrição
Criar template reutilizável de rateio de custos usando fórmulas NCalc com validação de sintaxe e proteção contra injeção.
...
### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.RATEIO.CREATE`
- Multi-tenancy: Usuário vinculado a empresa

### Fluxo Principal

| Passo | Ator | Sistema |
|-------|------|---------|
| 1 | Clica em "Criar Template Rateio" | - |
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.RATEIO.CREATE |
| 3 | - | Exibe form: Nome*, Fórmula* (NCalc), Dimensões (multi-select), fl_ativo |
| 4 | Preenche fórmula NCalc (ex: TotalValor / NumeroUsuarios) | - |
| 5 | - | Valida sintaxe NCalc (expressões permitidas) |
| 6 | - | Salva template com EmpresaId |
| 7 | - | Registra auditoria FIN_TEMPLATE_CRIADO |

### Fluxos de Exceção

**FE-UC09-01: Fórmula NCalc inválida**
- Condição: Sintaxe incorreta ou tentativa de injeção
- Resultado: Erro 400 com mensagem "Fórmula NCalc inválida"

### Regras de Negócio Aplicáveis
- RN-RF030-011: Validação de expressões NCalc
...
### Descrição
Executar simulação de rateio usando template com dados mock, exibindo preview sem salvar no banco.
...
### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.RATEIO.VIEW`
- Template de rateio existente

### Fluxo Principal

| Passo | Ator | Sistema |
|-------|------|---------|
| 1 | Seleciona template rateio | - |
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.RATEIO.VIEW |
| 3 | Fornece valores de entrada (mock data) | - |
| 4 | - | Executa fórmula NCalc em modo sandbox |
| 5 | - | Exibe resultado simulado (preview, não salva) |
| 6 | - | Exibe breakdown por dimensão (Centro Custo, Departamento) |

### Fluxos de Exceção

**FE-UC10-01: Erro de execução NCalc**
- Condição: Divisão por zero, overflow
- Resultado: Erro 400 com mensagem "Cálculo falhou"
...
### Descrição
Criar parâmetro fiscal com validação de vigência e alíquota dentro de range permitido.
...
### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.FISCAL.CREATE`
- Multi-tenancy: Usuário vinculado a empresa
...
| Passo | Ator | Sistema |
|-------|------|---------|
| 1 | Clica em "Criar Parâmetro Fiscal" | - |
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.FISCAL.CREATE |
| 3 | - | Exibe form: UF*, CFOP*, Alíquota*, Vigência_Início*, Vigência_Fim, fl_ativo |
| 4 | Preenche dados fiscais obrigatórios | - |
| 5 | - | Valida período vigência (início <= fim) |
| 6 | - | Valida alíquota (0-100%) |
| 7 | - | Salva parâmetro com EmpresaId |
| 8 | - | Registra auditoria FIN_FISCAL_CRIADO |

### Fluxos de Exceção

**FE-UC11-01: Período vigência inválido**
- Condição: Data fim < data início
- Resultado: Erro 400 com mensagem "Período de vigência inválido"

**FE-UC11-02: Alíquota fora do range**
- Condição: Alíquota < 0% ou > 100%
- Resultado: Erro 400 com mensagem "Alíquota deve estar entre 0% e 100%"

### Regras de Negócio Aplicáveis
- RN-RF030-012: Validação período vigência
- RN-RF030-013: Validação alíquota

---

## UC12 - Atualizar Parâmetro Fiscal

### Descrição
Atualizar parâmetro fiscal com versionamento automático e validação de impacto em faturas vigentes.
...
### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.FISCAL.UPDATE`
- Parâmetro fiscal existente

### Fluxo Principal

| Passo | Ator | Sistema |
|-------|------|---------|
| 1 | Clica em "Editar Parâmetro Fiscal" | - |
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.FISCAL.UPDATE |
| 3 | - | Verifica se parâmetro está em uso (faturas vigentes) |
| 4 | - | Exibe modal confirmação (impacto em X faturas) |
| 5 | Confirma alteração | - |
| 6 | - | Cria versionamento (snapshot histórico) |
| 7 | - | Salva nova versão |
| 8 | - | Registra auditoria FIN_FISCAL_EDITADO |

### Fluxos de Exceção

**FE-UC12-01: Usuário cancela operação**
- Condição: Usuário clica em "Cancelar" no modal de confirmação
- Resultado: Operação cancelada

### Regras de Negócio Aplicáveis
- RN-RF030-010: Versionamento automático
- RN-RF030-013: Validação alíquota
...
### Descrição
Homologar fornecedor validando checklist de documentos obrigatórios antes de liberar para faturamento.

### Atores
- Usuário autenticado com permissão de homologação
- Administrador financeiro
- Analista de compras

### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.FORNECEDOR.HOMOLOGAR`
- Fornecedor cadastrado em RF022
...
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.FORNECEDOR.HOMOLOGAR |
| 3 | - | Exibe checklist homologação (documentos, certidões) |
| 4 | Anexa documentos obrigatórios (PDF) | - |
| 5 | - | Valida completude (todos itens checklist OK) |
| 6 | - | Atualiza status fornecedor (Homologado) |
| 7 | - | Registra auditoria FIN_FORNECEDOR_HOMOLOGADO |

### Fluxos de Exceção

**FE-UC13-01: Checklist incompleto**
- Condição: Documentos obrigatórios faltando
- Resultado: Erro 400 com mensagem "Documentos obrigatórios faltando"

### Regras de Negócio Aplicáveis
- RN-RF030-014: Validação checklist homologação
...
### Descrição
Exibir dashboard em tempo real com KPIs de qualidade de parâmetros de faturamento usando cache Redis e SignalR.
...
### Pré-condições
- Usuário logado no sistema
- Usuário com permissão `FIN.PARAM_FATURAMENTO.DASHBOARD.VIEW`
...
| Passo | Ator | Sistema |
|-------|------|---------|
| 1 | Acessa "Dashboard Qualidade" | - |
| 2 | - | Valida permissão FIN.PARAM_FATURAMENTO.DASHBOARD.VIEW |
| 3 | - | Carrega KPIs (cache Redis 5min TTL) |
| 4 | - | Exibe cards: Taxa Rejeição, Tempo Médio Correção, Itens Pendentes, Tendência Qualidade |
| 5 | - | Exibe gráfico tempo real (SignalR, atualização <5s) |
| 6 | - | Exibe drill-down por categoria (clique em card) |

### Fluxos Alternativos

**FA-UC14-01: Filtrar por período**
- Passo 3a: Usuário seleciona período (últimos 7d, 30d, 90d)
- Passo 3b: Sistema recarrega KPIs com filtro aplicado

**FA-UC14-02: Exportar relatório**
- Passo 6a: Usuário clica em "Exportar"
- Passo 6b: Sistema gera PDF/Excel com dados do dashboard

### Regras de Negócio Aplicáveis
- RN-RF030-009: Cache Redis com TTL 5min
...
| 2.0 | 31/12/2025 | Architect Agent | Adicionados UC09-UC14 (Templates Rateio, Parâmetros Fiscais, Homologação, Dashboard) |
