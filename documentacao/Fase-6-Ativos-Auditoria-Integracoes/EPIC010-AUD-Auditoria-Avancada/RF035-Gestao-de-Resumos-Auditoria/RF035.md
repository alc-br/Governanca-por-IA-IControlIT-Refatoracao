...
11. **Segurança Row-Level Automática** - Isolamento por Fornecedor (ClienteId obrigatório) com validação em TODAS operações CRUD, RLS SQL Server, CONTEXT_INFO (**CRÍTICA**)
...
**Descrição:** Sistema DEVE aplicar isolamento automático por ClienteId em TODAS as operações CRUD. Usuários DEVEM acessar APENAS resumos do seu próprio Fornecedor.
...
**Descrição:** Sistema DEVE validar que: lote (FK) existe no banco, TODOS os itens pertencem ao mesmo Fornecedor do resumo, período do resumo coincide com período dos itens agrupados.
